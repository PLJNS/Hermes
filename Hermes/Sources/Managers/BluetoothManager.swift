...
class BluetoothManager: NSObject {
    ...
    static let defaultRestoreIdentifier = "prolificinteractive.Hermes.CBCentralManagerOptionRestoreIdentifierKey"
    ...
    init(delegate: BluetoothManagerDelegate, restoreIdentifier: String) {
...
extension BluetoothManager: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, willRestoreState dict: [String : Any]) {
        if let peripheralsObject = dict[CBCentralManagerRestoredStatePeripheralsKey] {
            if let restoredPeripherals = peripheralsObject as? [CBPeripheral] {
                restoredPeripherals.forEach({ peripherals.insert($0) })
                delegate?.bluetoothManager(self, didUpdateRankedPeripherals: rankedPeripherals)
            }
        }
    }
